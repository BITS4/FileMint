...
export function CropOverlay({
  ...
  const px = useMemo(() => {
    const toPx = (point: CropPoint) => ({ x: point.x * layout.width, y: point.y * layout.height });
    return { tl: toPx(quad.tl), tr: toPx(quad.tr), br: toPx(quad.br), bl: toPx(quad.bl) };
  }, [layout.height, layout.width, quad]);
  const mids = useMemo(
    () => ({
  ...
  const beginDrag = useCallback(
    ...
    [hitTest, onDragStateChange, quad],
  );
  ...
  const updateDrag = useCallback(
    ...
    [layout.height, layout.width, mode, onChange],
  );
