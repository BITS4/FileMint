...
describe('conversion server discovery', () => {
  beforeEach(() => {
    ...
    mocks.settings.serverUrl = 'http://localhost:8788';
  ...
  it('returns a local health response with absent capabilities safely defaulted', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      ...
      return url.startsWith('http://localhost:8788')
        ? health({ ocr: true })
  ...
  it('chooses the reachable LAN server advertising the strongest capability set', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (url === 'http://localhost:8788/health') return health({ ocr: true }, 'configured');
  ...
  it('discovers a development host from Expo metadata on Android', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn(async (input: string | URL | Request) =>
        String(input) === 'http://10.10.0.5:8788/health'
    ...
    expect(mocks.settings.update).toHaveBeenLastCalledWith({ serverUrl: 'http://10.10.0.5:8788' });
  });
