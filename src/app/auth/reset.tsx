  ...
  return (
    <Screen scroll padded contentContainerStyle={styles.screen}>
      ...
      <Card style={styles.card}>
        ...
        {step === 'confirm' ? (
          <>
            <TextField label="Reset code" icon="numeric" value={code} onChangeText={setCode} keyboardType="number-pad" maxLength={6} placeholder="123456" />
            <TextField
              label="New password"
              icon="lock-outline"
              value={password}
        ...
        {step === 'request' ? (
          <Button title="Send reset code" icon="email-send-outline" size="lg" full loading={loading} disabled={!email} onPress={requestCode} />
        ) : (
          <Button title="Change password" icon="check" size="lg" full loading={loading} disabled={!email || code.length !== 6 || !password} onPress={confirm} />
        )}
