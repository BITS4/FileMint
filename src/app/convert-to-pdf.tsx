...
import { baseName, extFromName, kindMeta, withExt } from '@/lib/format';
...
function outputPageBoxForRaster(
...
async function editedPreviewImage(
  page: StudioPage,
  previewBytes: Uint8Array,
...
export default function ConvertToPdfScreen() {
      ...
      for (let i = 0; i < nextFiles.length; i++) {
        ...
        const pageSizes = await Promise.all(
          rendered.map((_, index) =>
            getPdfPageSize(sourceBytes, index).catch(() => ({ width: 595.28, height: 841.89 })),
          ),
        );
  ...
  const updateCurrentCrop = (all: boolean) => {
    ...
    setPages((prev) =>
      prev.map((page) => (all || page.id === selectedPage?.id ? { ...page, crop, quad } : page)),
    );
  };
  ...
  const applyFilter = (filter: FilterId, all: boolean) => {
    setPages((prev) =>
      prev.map((page) => (all || page.id === selectedPage?.id ? { ...page, filter } : page)),
    );
  };
